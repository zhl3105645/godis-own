...
// item stores roles for compile in order
type item struct {
	character byte
	set       map[byte]bool
	typeCode  int
...
// contains represents whether c match setSymbol, rangSymbol, negSymbol
...
func (p *Pattern) IsMatch(s string) bool {
	...
	m := len(s)
	n := len(p.items)
	// table[i][j] represents len(s) = i match the len(p.items) = j
	table := make([][]bool, m+1)
	for i := 0; i < m+1; i++ {
